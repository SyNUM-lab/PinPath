...
drawGPML <- function(infile,
  ...
  groupElements_df <- .prepareLabels(gpml_fil[names(gpml_fil) %in% c("DataNode", "Label")])
  ...
  par(mar = c(0,0,0,0),#c(5.1,4.1,4.1,2.1), 
      lheight=0.9)
  ...
  for (i in ZOrder_df$Index){
    ...
    if (names(pathwayElement) == "DataNode"){
      ...
      if (!(is.null(geneIDs) | is.null(colorVar) | (is.null(annGenes) & is.null(annMetabolites)) | is.null(inputDB))){
        ...
        # Select color values of oathway element
        colors_df <- colors_df_all[colors_df_all$GraphId1 == pathwayElement[[1]]$ID,]
    ...
    if (names(pathwayElement) == "Shape"){
      ...
      non_cellcomp_df <- shapes_df[!(shapes_df$ShapeType %in% c("Mitochondria",
      ...
      cellcomp_df <- shapes_df[shapes_df$ShapeType %in% c("Mitochondria",
                                                          "Sarcoplasmic Reticulum",
                                                          "Endoplasmic Reticulum",
                                                          "Golgi Apparatus"),]
    ...
    if (names(pathwayElement) == "Interaction" ){
      ...
      # Prepare edges
      edges_df <- .prepareEdges(pathwayElement, gpml_fil[names(gpml_fil) %in% c("Interaction", "GraphicalLine")])
  ...
  if (nodeTable & !is.null(colors_df_all)){
    outputTable <- unique(colors_df_all[!is.na(colors_df_all$ScaleName),c("Label", "InputId", "ScaleName", "MapColor")])
