...
#' @param dataEdges A GPML list object filtered for edges.
#' @return A data frame for plotting edges.

.prepareEdges <- function(dataEdges, dataEdges_all){
  
  # First define a function that can extract the information from the 
  # gpml list and put it into a data frame
  # dataEdges: single entry of gpml edge list
  # dataEdges_all: all entries of gpml edge list
  # anchorIds: vector with the graph IDs from anchor points
    ...
    if (sum(names(dataEdges$Graphics) == "Point") > 2){
      ...
      for (p in seq_len(sum(names(dataEdges$Graphics) == "Point")-1)){
        ...
        if (p == 1){
          ...
          temp_edges1$Xrel1 <- sign(temp_edges1$Xrel1)*round(abs(temp_edges1$Xrel1)-0.01)
          temp_edges1$Yrel1 <- sign(temp_edges1$Yrel1)*round(abs(temp_edges1$Yrel1)-0.01)
          ...
          if ((temp_edges1$Xrel1 == 0) & (temp_edges1$Yrel1 == 0)){
            ...
            attachedEdges <- as.numeric(which(unlist(lapply(anchorIds, function(x) temp_edges1$GraphRef1 %in% x))))
          ...
          temp_edges2 <- data.frame(
            ...
            Xrel2 = -1*floor(abs(temp_edges1$Yrel1))*sign(as.numeric(dataEdges$Graphics[[p+1]]["X"])- temp_edges1$X1),
            Yrel2 = -1*floor(abs(temp_edges1$Xrel1))*sign(as.numeric(dataEdges$Graphics[[p+1]]["Y"])- temp_edges1$Y1),
            ArrowHead2 = as.character(dataEdges$Graphics[[p+1]]["ArrowHead"]),
            LineStyle = tolower(as.character(ifelse(is.na(dataEdges$Graphics$.attrs["LineStyle"]),
                                                    "solid", dataEdges$Graphics$.attrs["LineStyle"]))),
            Color = as.character(ifelse(is.na(dataEdges$Graphics$.attrs["Color"]),
                                        "#000000", paste0("#",dataEdges$Graphics$.attrs["Color"]))),
            ConnectorType = as.character(ifelse(is.na(dataEdges$Graphics$.attrs["ConnectorType"]),
                                                "Straight", dataEdges$Graphics$.attrs["ConnectorType"])),
            LineThickness = as.numeric(ifelse(is.na(dataEdges$Graphics$.attrs["LineThickness"]),
        ...
        if(!is.na(temp_edges$ArrowHead1) & !is.na(temp_edges$ArrowHead2)){
          ...
          temp_edges$ArrowType <- ifelse(temp_edges$ArrowHead1 == temp_edges$ArrowHead2,
      ...
      if ((edges_df$Xrel1 == 0) & (edges_df$Yrel1 == 0)){
        ...
        # Get the edge(s) to which the edge is attached
        attachedEdges <- as.numeric(which(unlist(lapply(anchorIds, function(x) edges_df$GraphRef1 %in% x))))
      ...
      if ((edges_df$Xrel2 == 0) & (edges_df$Yrel2 == 0)){
        ...
        # Get the edge(s) to which the edge is attached
        attachedEdges <- as.numeric(which(unlist(lapply(anchorIds, function(x) edges_df$GraphRef2 %in% x))))
  ...
  if (nrow(edges_df[edges_df$ConnectorType == "Straight" |
    ...
    edges_df_straight <- edges_df[edges_df$ConnectorType == "Straight" |
                                    edges_df$ConnectorType == "Segmented",]
    edges_df_straight$LineStyle[edges_df_straight$LineStyle == "broken"] <- "dashed"
  } else{
  ...
  if (nrow(edges_df[edges_df$ConnectorType == "Elbow",]) > 0){
    edges_df_elbow <- .prepareElbow(edges_df[edges_df$ConnectorType == "Elbow",])
  } else{
    edges_df_elbow <- NULL
  }
  ...
  if (nrow(edges_df[edges_df$ConnectorType == "Curved",]) > 0){
    edges_df_curved <- .prepareCurve(edges_df[edges_df$ConnectorType == "Curved",])
  } else{
  ...
  if (sum(edges_df_all$ArrowType == "Different")>0){
    ...
    edges_df_all <- rbind.data.frame(edges_df_all[edges_df_all$ArrowType != "Different",],
                                     edges_df_dif)
  }
  ...
  if (sum(edges_df_all$ArrowEnd == "both")>0){
    ...
    edges_df_dif <- rbind.data.frame(edges_df_dif1, edges_df_dif2)
    edges_df_all <- rbind.data.frame(edges_df_all[edges_df_all$ArrowEnd != "both",],
                                     edges_df_dif)
  }
...
#' @param df A data frame with edge information, as generated by the .prepareEdges() function.
  ...
  if (sum(df$ArrowType == "mim-gap") > 0){
    ...
    for (a in seq_len(nrow(plotDF_temp))){
      ...
      if (plotDF_temp$ArrowEnd[a] == "first"){
        ...
        Ystart <- -1*(y1[a]-gap*((y1[a]-y2[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a]))))
        Yend <- -y2[a]
      ...
      if (plotDF_temp$ArrowEnd[a] == "last"){
        Xstart <- x1[a]
        Xend <- x2[a]-gap*((x2[a]-x1[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a])))
        
        Ystart <- -y1[a]
        Yend <- -1*(y2[a]-gap*((y2[a]-y1[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a]))))
  ...
  if (sum(df$ArrowType == "mim-cleavage")> 0){
    ...
    for (a in seq_len(nrow(plotDF_temp))){
      ...
      if (plotDF_temp$ArrowEnd[a] == "first"){
        ...
        rotated_coords <- t(.rotation_matrix(-0.5*pi) %*% c(deviation*((x1[a]-x2[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a]))),
      ...
      if (plotDF_temp$ArrowEnd[a] == "last"){
        ...
        # Get orthogonal part of the line
        rotated_coords <- t(.rotation_matrix(-0.5*pi) %*% c(deviation*((x2[a]-x1[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a]))),
  ...
  if (sum(df$ArrowType == "mim-transcription-translation")> 0){
    ...
    for (a in seq_len(nrow(plotDF_temp))){
      ...
      if (plotDF_temp$ArrowEnd[a] == "first"){
        ...
        # Get main part of line
        Xstart <- x1[a]-(gap-extend)*((x1[a]-x2[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a])))
        Xend <- x2[a]
        Ystart <- -1*(y1[a]-(gap-extend)*((y1[a]-y2[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a]))))
        Yend <- -y2[a]
      ...
      if (plotDF_temp$ArrowEnd[a] == "last"){
        ...
        # Get main part of line
        Xstart <- x1[a]
        Xend <- x2[a]-(gap-extend)*((x2[a]-x1[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a])))
        Ystart <- -y1[a]
        Yend <- -1*(y2[a]-(gap-extend)*((y2[a]-y1[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a]))))
        ...
        Xstart <- x2[a]-(gap)*((x2[a]-x1[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a])))
        Ystart <- -1*(y2[a]-(gap)*((y2[a]-y1[a])/(abs(x1[a]-x2[a]) + abs(y1[a]-y2[a]))))
        Xorth_end <- Xstart + rotated_coords[1,1]
        Yorth_end <- Ystart + rotated_coords[1,2]
