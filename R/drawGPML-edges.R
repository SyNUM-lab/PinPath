    ...
    if (sum(names(dataEdges$Graphics) == "Point") > 2){
        ...
        for (p in seq_len(sum(names(dataEdges$Graphics) == "Point")-1)){
            ...
            if (p == 1){
                ...
                # Collect the information about the starting point of the 
                # first edge
                    ...
                    attachedEdges <- as.numeric(
                        which(
                            unlist(
                                lapply(
                                    anchorIds, 
                                    function(x) temp_edges1$GraphRef1 %in% x
                                )
                            )
                        )
                    )
                ...
                # Collect the information about the end point of the first 
                # edge
            ...
            attachedEdges <- as.numeric(
                which(
                    unlist(
                        lapply(
                            anchorIds, 
                            function(x) edges_df$GraphRef1 %in% x
                        )
                    )
                )
            )
            ...
            attachedEdges <- as.numeric(
                which(
                    unlist(
                        lapply(
                            anchorIds, 
                            function(x) edges_df$GraphRef2 %in% x
                        )
                    )
                )
            )
...
.drawEdges <- function(df){
    ...
    if (sum(
        ...
        # arrow head, because the arrow head looks weird when the line is 
        # dashed.
    ...
    if (sum(
        df$ArrowType == "mim-stimulation")>0){
