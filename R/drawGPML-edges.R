...
.prepareEdges <- function(dataEdges, dataEdges_all){
    ...
    if (nrow(edges_df[edges_df$ConnectorType == "Curved",]) > 0){
        ...
        }else{edges_df_curved <- .prepareCurve(
            edges_df[edges_df$ConnectorType == "Curved",])}
    } else{edges_df_curved <- NULL}
    edges_df_all <- rbind.data.frame(edges_df_straight, edges_df_elbow)

    # If arrow head is on both sides, we need to draw them as two arrows
    edges_df_all <- .separateDoubleArrows(edges_df_all)
    return(list(lines = edges_df_all, curves = edges_df_curved))
}

.separateDoubleArrows <- function(edges_df_all){
...
.edgeFUN <- function(dataEdges, dataEdges_all, anchorIds){
    ...
    if (sum(names(dataEdges$Graphics) == "Point") > 2){
        edges_df <- .edgeFUN_custom(dataEdges, dataEdges_all, anchorIds)
    } else{
        edges_df <- .edgeFUN_default(dataEdges, dataEdges_all, anchorIds)
    }
...
.edgeFUN_custom <- function(dataEdges, dataEdges_all, anchorIds){
    ...
    for (p in seq_len(sum(names(dataEdges$Graphics) == "Point")-1)){
        ...
        # follow from this.
        if (p == 1){
            temp_edges <- .edgeFUN_custom_edge1(
                dataEdges, dataEdges_all, anchorIds, p)
        } else{
            # For the coordinates after the first edge, the relative X and
            # Y positions can be determined from the edge before
        ...
        # data.frame
        temp_edges <- .edgeFUN_AddArrowHead(temp_edges)
...
.edgeFUN_custom_edge1 <- function(dataEdges, dataEdges_all, anchorIds, p){
    # Collect the information about the starting point of the first edge
    temp_edges1 <- .collectEdges_custom_start(dataEdges, p)

    # If the relative position of X and Y is both zero, the edge is probably
    # attached to another edge. So, we need to determine the relative X and Y
    ...
    if ((temp_edges1$Xrel1 == 0) & (temp_edges1$Yrel1 == 0)){
        ...
        if(length(attachedEdges) != 0){
            ...
            # If the attached edge moves predominantly in
            # Y direction, the attachment point should be in the
            # X direction
            if (abs(dY) > abs(dX)){
                if (temp_edges1$X1 > as.numeric(
                    dataEdges$Graphics[[p+1]]["X"])){
                    temp_edges1$Xrel1 <- -1
                } else{temp_edges1$Xrel1 <- 1}}
            # If the attached edge moves predominantly in X
            # direction, the attachment point should be in the
            # Y direction
            if (abs(dY) <= abs(dX)){
                if (temp_edges1$Y1 > as.numeric(
                    dataEdges$Graphics[[p+1]]["Y"])){
                    temp_edges1$Yrel1 <- -1
                } else{temp_edges1$Yrel1 <- 1}}
    ...
    temp_edges2 <- .collectEdges_custom_end(dataEdges,temp_edges1, p)
...
.collectEdges_custom_start <- function(dataEdges, p){
    temp_edges1 <- data.frame(
        X1 = as.numeric(dataEdges$Graphics[[p]]["X"]),
        Y1 = as.numeric(dataEdges$Graphics[[p]]["Y"]),
        GraphRef1 = as.character(
            dataEdges$Graphics[[p]]["GraphRef"]),
        Xrel1 = ifelse(
            is.na(as.numeric(dataEdges$Graphics[[p]]["RelX"])),
            -1, as.numeric(dataEdges$Graphics[[p]]["RelX"])),
        Yrel1 = ifelse(
            is.na(as.numeric(dataEdges$Graphics[[p]]["RelY"])),
            0, as.numeric(dataEdges$Graphics[[p]]["RelY"])),
        ArrowHead1 = as.character(
            dataEdges$Graphics[[p]]["ArrowHead"]))
...
.edgeFUN_custom_edge2 <- function(
    ...
    temp_edges2 <- data.frame(
        ...
        GraphRef2 = as.character(
            dataEdges$Graphics[[p+1]]["GraphRef"]),
        Xrel2 = -1*floor(abs(temp_edges1$Yrel1))*sign(
            as.numeric(dataEdges$Graphics[[p+1]]["X"]) -
                temp_edges1$X1),
        Yrel2 = -1*floor(abs(temp_edges1$Xrel1))*sign(
            as.numeric(dataEdges$Graphics[[p+1]]["Y"]) -
                temp_edges1$Y1),
        ArrowHead2 = as.character(
            dataEdges$Graphics[[p+1]]["ArrowHead"]),
...
.edgeFUN_AddArrowHead <- function(temp_edges){
...
.edgeFUN_default <- function(dataEdges, dataEdges_all, anchorIds){

    # Extract edge information from dataEdges list
    edges_df <- .collectEdges_default(dataEdges)
    ...
    if ((edges_df$Xrel1 == 0) & (edges_df$Yrel1 == 0)){
        edges_df <- .positionAttachedEdges_start(
            edges_df, dataEdges_all, anchorIds)
    }

    # If the relative position of ending X (Xrel2) and Y (Yrel2) is
    # both zero, the edge is probably attached to another edge.
    # So, we need to determine the relative X and Y position from
    # the attached edge
    if ((edges_df$Xrel2 == 0) & (edges_df$Yrel2 == 0)){
        edges_df <- .positionAttachedEdges_end(
            edges_df, dataEdges_all, anchorIds)
    }

    # Now we add information about the arrow head to the dataframe
    edges_df <- .edgeFUN_AddArrowHead(edges_df)
    return(edges_df)
}


.collectEdges_default <- function(dataEdges){
    ...
    edges_df <- data.frame(
        ...
        Xrel1 = ifelse(
            is.na(as.numeric(dataEdges$Graphics[[1]]["RelX"])),
            -1, as.numeric(dataEdges$Graphics[[1]]["RelX"])),
        Yrel1 = ifelse(
            is.na(as.numeric(dataEdges$Graphics[[1]]["RelY"])),
        ...
        Xrel2 = ifelse(
            is.na(as.numeric(dataEdges$Graphics[[2]]["RelX"])),
            1, as.numeric(dataEdges$Graphics[[2]]["RelX"])),
        Yrel2 = ifelse(
            is.na(as.numeric(dataEdges$Graphics[[2]]["RelY"])),
...
.positionAttachedEdges_start <- function(edges_df, dataEdges_all, anchorIds){
    # Get the edge(s) to which the edge is attached
    attachedEdges <- as.numeric(which(unlist(lapply(
        anchorIds, function(x) edges_df$GraphRef1 %in% x))))
    ...
    if(length(attachedEdges) != 0){
        ...
        if (abs(dY) > abs(dX)){
            if (edges_df$X1 > edges_df$X2){
                edges_df$Xrel1 <- -1}else{edges_df$Xrel1 <- 1}}
...
.positionAttachedEdges_end <- function(edges_df, dataEdges_all, anchorIds){
    # Get the edge(s) to which the edge is attached
    attachedEdges <- as.numeric(which(unlist(lapply(
        anchorIds,function(x) edges_df$GraphRef2 %in% x))))
    ...
    if(length(attachedEdges) != 0){
        ...
        if (abs(dY) > abs(dX)){
            if (edges_df$X2 > edges_df$X1){
                edges_df$Xrel2 <- -1}else{edges_df$Xrel2 <- 1}}

        # If the attached edge moves predominantly in X direction,
        # the attachment point should be in the Y direction
        if (abs(dY) <= abs(dX)){
            if (edges_df$Y2 > edges_df$Y1){
                edges_df$Yrel2 <- -1} else{edges_df$Yrel2 <- 1}}
...
.drawEdges <- function(df){
    ...
    if (sum(df$ArrowType %in% sel) > 0){
        ...
        .drawEdgeTriangle(plotDF_temp)}
    # Open arrow head
    sel <- c("mim-binding", "mim-modification")
    if (sum(df$ArrowType %in% sel) > 0){
        plotDF_temp <- df[df$ArrowType %in% sel,]
        .drawEdgeOpenArrow(plotDF_temp)}
    # Filled white arrow
    sel <- c("mim-stimulation")
    if (sum(df$ArrowType %in% sel) > 0){
        plotDF_temp <- df[df$ArrowType %in% sel,]
        .drawEdgeWhiteTriangle(plotDF_temp)}
    # T-bar
    ...
    if (sum(df$ArrowType %in% sel) > 0){
        plotDF_temp <- df[df$ArrowType %in% sel,]
        .drawEdgeWhiteCircle(plotDF_temp)}
    # Filled white square
    sel <- c("mim-covalent-bond")
    if (sum(df$ArrowType %in% sel) > 0){
        plotDF_temp <- df[df$ArrowType %in% sel,]
        .drawEdgeWhiteSquare(plotDF_temp)}
    ...
    if (sum(df$ArrowType %in% sel) > 0){
        plotDF_temp <- df[df$ArrowType %in% sel,]
        .drawEdgeCleavage(plotDF_temp)}
    # `mim-transcription-translation`
    sel <- c("mim-transcription-translation")
    if (sum(df$ArrowType %in% sel) > 0){
        plotDF_temp <- df[df$ArrowType %in% sel,]
        .drawEdgeTrans(plotDF_temp)}
}

.drawEdgeLine <- function(plotDF){
    graphics::arrows(
        x0 = plotDF$X1, x1 = plotDF$X2,
...
.drawEdgeTriangle <- function(plotDF_temp){
    plotDF <- .prepareEdgeTriangle(plotDF_temp)

    # Draw edges
    graphics::arrows(
        x0 = plotDF$Xstart, x1 = plotDF$Xend,
...
.prepareEdgeTriangle <- function(plotDF_temp){
...
.drawEdgeOpenArrow <- function(plotDF_temp){
    plotDF <- .prepareEdgeOpenArrow(plotDF_temp)
...
.drawEdgeWhiteTriangle <- function(plotDF_temp){
    plotDF <- .prepareEdgeWhiteTriangle(plotDF_temp)
...
.drawEdgeTBar <- function(plotDF_temp){

    plotDF <- .prepareEdgeTBar(plotDF_temp)
...
.drawEdgeWhiteCircle <- function(plotDF_temp){

    plotDF <- .prepareEdgeWhiteCircle(plotDF_temp)
...
.drawEdgeGap <- function(plotDF_temp){
    plotDF<- .prepareEdgeGap(plotDF_temp)
...
.drawEdgeCleavage <- function(plotDF_temp){

    plotList <- .prepareEdgeCleavage(plotDF_temp)
    ...
    # Draw edges
    graphics::arrows(
        x0 = plotDF_main$X1, x1 = plotDF_main$X2,
...
.drawEdgeTrans <- function(plotDF_temp){

    plotList <- .prepareEdgeTrans(plotDF_temp)
    plotDF_main <- plotList[[1]]
    plotDF_orth <- plotList[[2]]
