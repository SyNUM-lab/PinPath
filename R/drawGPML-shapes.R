    ...
    shapeFUN <- function(dataShapes){
        data.frame(
            ...
            Valign = ifelse(
                ...
                ifelse(
                    as.character(
                        dataShapes$Graphics["Valign"]) == "Top", 
                    1,0)
            ), 
            Align = ifelse(
            ...
            nLine = ifelse(
                sum(stringr::str_detect(
                    unlist(
                        dataShapes[
                            which(names(dataShapes) == "Attribute")]
                    ), 
                    "Double")) == 0, 
                "Single", "Double"),
...
.drawBraces <- function(braces_df){
    ...
    for (i in seq_len(nrow(braces_df))){
        plot_all <- rbind.data.frame(plot_all, .braceCoord(braces_df, i, npoints))}
