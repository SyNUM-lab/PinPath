  ...
  if (length(scales) > 0){
    for (s in 1:length(scales)){
      ...
      if (ScaleType == "Divergent"){
        ...
        pal_up <- grDevices::colorRampPalette(colors = c(colorList[[s]]$Color["MidCol"],
                                                         colorList[[s]]$Color["MaxCol"]))(101)
        
        # Make color gradient from min to middle value
        pal_down <- grDevices::colorRampPalette(colors = c(colorList[[s]]$Color["MidCol"], 
  ...
  for (s in 1:nScales){
    ...
    if (ScaleType == "Divergent"){
      ...
      grid::grid.rect(x = seq(0.1,0.9,length = 81*2) + 1,
                      ...
                      gp = grid::gpar(col = NA,
                                      fill = pal(81*2)))
      graphics::arrows(x0 = 1/(2*nScales) + (s-1)*1/nScales, x1 =1/(2*nScales) + (s-1)*1/nScales,
                       y0 = 0.25, y1 = (s-1)-nScales, code = 0)
      graphics::arrows(x0 = 1/(2*nScales) + (s-1)*1/nScales, x1 = 1.1,
