...
#' @param annGenes Bioconductor annotation package.
...
.mapColors <- function(nodes_df,
  ...
  geneIDs_fil$Database <- replace(geneIDs_fil$Database,
                                  geneIDs_fil$Database == "Ensembl","ENSEMBL")
  geneIDs_fil$Database <- replace(geneIDs_fil$Database,
                                  geneIDs_fil$Database == "Entrez Gene","ENTREZID")
  geneIDs_fil$Database <- replace(geneIDs_fil$Database,
                                  geneIDs_fil$Database == "Uniprot-TrEMBL","UNIPROT")
  geneIDs_fil$Database <- replace(geneIDs_fil$Database,
                                  geneIDs_fil$Database == "HGNC","SYMBOL")
  ...
  for (db in unique(inputDB[inputDB %in% geneDBs_name])){
    ...
    for (i in 1:length(databases)){
      ...
      if (keytype == db){
        ...
        temp <- tryCatch({
          temp <- geneIDs_fil[geneIDs_fil$Database == keytype, ]
            ann <- suppressMessages(AnnotationDbi::select(BiocGenerics::get(annGenes), 
    ...
    nodeAnn <- nodeAnn[,c("GraphId1", "InputId", "Col", "Scale", "ScaleName")]
    colnames(nodeAnn) <- c("GraphId1", "InputId","MapColor", "Scale", "ScaleName")
  ...
  # If one node has at least 1 non-NA color value, remove the NA values from different inputIds:
  
  # Nodes with non-NA values
  nonNA_nodes <- colors_df$GraphId1[!is.na(colors_df$MapColor)] 
  
  # all node-id combinations
  node_ids <- paste0(colors_df$InputId[colors_df$GraphId1 %in% nonNA_nodes], "_",
  ...
  colors_df <- colors_df[!(paste0(colors_df$InputId, "_", colors_df$GraphId1) %in% rmIds),]
  ...
  if (length(scales) > 0){
    for (s in 1:length(scales)){
      ...
      if (ScaleType == "Divergent"){
        ...
        pal_up <- colorRampPalette(colors = c(colorList[[s]]$Color["MidCol"],
                                              colorList[[s]]$Color["MaxCol"]))(101)
        
        # Make color gradient from min to middle value
        pal_down <- colorRampPalette(colors = c(colorList[[s]]$Color["MidCol"], 
...
#' @param colors_df A data frame with edge information, as generated by the .mapColors() function.
  ...
  for (s in 1:nScales){
    ...
    if (ScaleType == "Divergent"){
      ...
      grid::grid.rect(x = seq(0.1,0.9,length = 81*2) + 1,
                      ...
                      gp = grid::gpar(col = NA,
                                fill = pal(81*2)))
      arrows(x0 = 1/(2*nScales) + (s-1)*1/nScales, x1 =1/(2*nScales) + (s-1)*1/nScales,
             y0 = 0.25, y1 = (s-1)-nScales, code = 0)
      arrows(x0 = 1/(2*nScales) + (s-1)*1/nScales, x1 = 1.1,
