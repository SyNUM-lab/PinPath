...
defaultColorList <- function(ColorVar, ColorNames = NULL){
  ...
  for (c in 1:ncol(ColorVar)){
    if (is.numeric(ColorVar[,c])){
      ...
      if ((min(ColorVar[,c], na.rm = TRUE) < 0) & (max(ColorVar[,c], na.rm = TRUE) > 0)){
        
        # Since we want to color scale to be symmetric, we set the absolute min and max value 
        # to the same max absolute value
        max_absolute_value <- max(abs(as.numeric(quantile(ColorVar[,c], 0.9, na.rm = TRUE))),
                                  abs(as.numeric(quantile(ColorVar[,c], 0.1, na.rm = TRUE))))
      ...
      if ((min(ColorVar[,c], na.rm = TRUE) >= 0) | (max(ColorVar[,c], na.rm = TRUE) <= 0)){
        ...
        colorList[[c]] <- list(
          ...
          ColorVal = c("MinVal" = as.numeric(quantile(ColorVar[,c],0.1, na.rm = TRUE)),
                       "MaxVal" = as.numeric(quantile(ColorVar[,c],0.9, na.rm = TRUE))),
    ...
    if (!is.numeric(ColorVar[,c])){
      ...
      if (length(unique(ColorVar[,c])) == 2){
        colorList[[c]] <- list(
          ...
          Color = setNames(c("green", "yellow"), unique(ColorVar[,c]))
        )
      } else{
        colorList[[c]] <- list(
          ScaleName = colnames(ColorVar)[c],
          ScaleType = "Qualitative",
          Color = setNames(rainbow(length(unique(ColorVar[,c]))), unique(ColorVar[,c]))
        )
      }
    }
  }
