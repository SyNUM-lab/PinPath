...
server <- function(input, output, session){
  ...
  observe({
    ...
    # Set tables to zero
    output$statTable_view <- renderDataTable(NULL)
    ...
    observe({
      req(rv$statTable)
      updateSelectizeInput(session, "color_column", 
    ...
    observeEvent(input$pathway, {
      ...
      proxy <- dataTableProxy("pathStats_view")
      selectRows(proxy, which(rv$pathStats[[input$collection]]$ID == rv$pathway))
    }, ignoreInit = TRUE)
    ...
    observe({
      ...
      tryCatch({
        ...
        if (input$collection == "WikiPathways"){
          ...
          if (input$asNetwork){
            rv$outPath <- PinPath::GPML2Network(
              infile = rWikiPathways::getPathway(rv$pathway),
              outdir = file_dir,
              outname = "pathway.svg",
              annGenes = pkg,
        ...
        if (input$collection == "KEGG"){
          ...
          if (input$asNetwork){
            bfc <- BiocFileCache::BiocFileCache()
            rv$outPath <- PinPath::KGML2Network(
              infile = BiocFileCache::bfcrpath(bfc, paste0("https://rest.kegg.jp/get/",rv$pathway,"/kgml")),
              outdir = file_dir,
              outname = "pathway.svg",
              annGenes = pkg,
    ...
    observe({
      ...
      if (length(setdiff(input$color_column, colnames(rv$statTable)))==0){
        rv$colorList <- PinPath::defaultColorList(rv$statTable[,input$color_column],
